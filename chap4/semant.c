...
struct expty transExp(S_table venv, S_table tenv, A_exp a) {
    switch(a->kind) {
        ...
        case A_callExp: {
            ...
            Ty_tyList t1 = fun->u.fun.formals;
            A_expList e1 = a->u.call.args;
            for ( ; t1 && e1; t1 = t1->tail, e1 = e1->tail) {
                struct expty exp = transExp(venv, tenv, e1->head);
                if (!cmp_ty(t1->head, exp.ty))
                    EM_error(a->pos, "argument type mismatch");
            }
            if (t1) {
                EM_error(a->pos, "few args");
            }
            else if (e1) {
                EM_error(a->pos, "too many args");
            }
            ...
            switch(oper) {
                ...
                case A_neqOp: {
                    if (left.ty->kind == Ty_void) {
                        EM_error(a->u.op.left->pos, "expression is no value");
                    }
                    else if (right.ty->kind == Ty_void) {
                        EM_error(a->u.op.right->pos, "expression is no value");
                    }
                    else if (!cmp_ty(left.ty, right.ty)) {
                        EM_error(a->u.op.right->pos, "comparison type not equal");
                    }
                    return expTy(NULL, Ty_Int());
                }
                case A_ltOp:
                ...
                case A_geOp: {
                    if (left.ty->kind != Ty_int && left.ty->kind != Ty_string) {
                        ...
                        EM_error(a->u.op.right->pos, "comparison type missmatch");
                    }
                    return expTy(NULL, Ty_Int());
                }
            }
        ...
        case A_recordExp: {
            ...
            for ( ; ti && ei; ti = ti->tail, ei = ei->tail) {
                ...
                if(!cmp_ty(ti->head->ty, exp.ty))
                    EM_error(a->pos, "member %s type missmatch", S_name(ti->head->name));
            }
            if(ti) 
                EM_error(a->pos, "few initializers for %s", S_name(a->u.record.typ));
            else if(ei) {
        ...
        case A_assignExp: {
            ...
            if(!cmp_ty(var.ty, exp.ty)){
                EM_error(a->pos, "assignment type missmatch");
            }
            return expTy(NULL, Ty_Void());
        }
        case A_ifExp: {
        ...
        case A_whileExp: {
            ...
            loop--;
            if(t.ty->kind != Ty_int) {
                EM_error(a->pos, "expression need an integer");
            }
            if(b.ty->kind != Ty_void){
                EM_error(a->pos, "expression shouldn't return value");
            }
            return expTy(NULL, Ty_Void());
        }
        case A_forExp: {
            ...
            loop--;
            if (l.ty->kind != Ty_int)
                EM_error(a->pos, "low bound need a integer");
            if (h.ty->kind != Ty_int)
                EM_error(a->pos, "high bound need a integer");
            if (b.ty->kind != Ty_void)
                EM_error(a->pos, "body expression should not have a return value");
            return expTy(NULL, Ty_Void());
        }
        case A_breakExp: {
            if(!loop) {
                EM_error(a->pos, "break within loop");
            }
            return expTy(NULL, Ty_Void());
        }
        case A_letExp: {
        ...
        case A_arrayExp: {
            ...
            if(!cmp_ty(i.ty, t->u.array))
                EM_error(a->pos, "array init type missmatch");
            return expTy(NULL, t);
        }
        }
    }
        ...
        case A_varDec: {
            ...
            if (d->u.var.typ) {
                ...
                if (!t) {
                    ...
                    if(!cmp_ty(t, e.ty))
                        EM_error(d->pos, "var init type missmatch");
            ...
            for(l = d->u.type; l; l=l->tail) {
                ...
                while (t && t->kind == Ty_name) {
                    if(!set_push(s, t->u.name.sym)) {
                        EM_error(d->pos, "illegal recurise definition %s", S_name(t->u.name.sym));
                        t->u.name.ty = Ty_Int();
                        break;
                    }
                    t = t->u.name.ty;
